...
%   figure, subplot(1,2,1), imshow(I,[]); subplot(1,2,2), imshow(Ivessel,[0 0.25]);
%
% Written by Marc Schrijver, 2/11/2001
% Re-Written by D.Kroon University of Twente (May 2009)

defaultoptions = struct('FrangiScaleRange', [1, 10], 'FrangiScaleRatio', 2, 'FrangiBetaOne', 0.5, 'FrangiBetaTwo', 15, 'verbose',true,'BlackWhite',true, 'magFilt', false);
...
for i = 1:length(sigmas),
    ...
    [Lambda2,Lambda1,Ix,Iy]=eig2image(Dxx,Dxy,Dyy); %lambda2 is greater
    
    
    
    % Compute the direction of the minor eigenvector
    angles = atan2(Ix,Iy);
%     disp(['angles is: ' mat2str(size(Ix))]);
...
%     Lambda1 = abs(Lambda1);
%     Lambda2 = abs(Lambda2);  %recently added

    Lambda2(Lambda2==0) = eps;
    if options.verbose
        figure;
        subplot(2,3,1); imagesc(Lambda2); colorbar; title('Lambda2');
        subplot(2,3,3); imagesc(Lambda1); colorbar; title('Lambda1');
        subplot(2,3,2); imagesc(I); colorbar; title('I');
        subplot(2,3,4); imagesc(Ix); colorbar; title('Ix');
        subplot(2,3,6); imagesc(Iy); colorbar; title('Iy');
    end

    magnitudeFilter = Lambda2 - Lambda1 / 2; %these two lines
    magnitudeFilter(magnitudeFilter>=0) = 0; %set where lambda2>lambda1/2 = 0 (sphere)
    magnitudeFilter(Iy<0.8) = 0; %remove non vertical eigenvectors
    magnitudeFilter(Ix<-.2) = 0; %remove horizontal eigenvectors
    magnitudeFilter(magnitudeFilter < 0) = 1; %normalize
    
    if options.magFilt
        'printed'
        Ifiltered = Ifiltered.* magnitudeFilter;
    else
        %end my code
%         see pp. 45
    ...
    ALLangles(:,:,i) = angles;
    if options.verbose
        figure; subplot(1,2,1); imagesc(magnitudeFilter); colorbar; title('magnitudeFilter');
        subplot(1,2,2); imagesc(Ifiltered); colorbar; title('filtered  + magnitude filter');
    end
end
...
if length(sigmas) > 1,
    ...
    if options.verbose
        figure; imagesc(outIm); colorbar; title('out');
    end
    if(nargout>1)
